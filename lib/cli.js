...
async function setup () {
  ...
  if (import.meta.dirname) {
    ...
    // Remove once Node18 goes EOL
    dirname = fileURLToPath(new URL('.', import.meta.url))
  } else {
    console.error('Failed to work out the current dirname.  Is this a supported version of Node.js?')
  }
  ...
  const responses = await inquirer.prompt([
    {
      type: 'input',
      name: 'logDir',
      default: `${path.join(dirname, '..', 'data', 'logs')}`,
      message: 'Enter a path for the log directory'
    },
    {
      type: 'input',
      name: 'dbDir',
      default: `${path.join(dirname, '..', 'data', 'db')}`,
  ...
  try {
    ...
    await fs.writeFile(path.join(dirname, '..', '.env'), envFileContent)
    console.log('\nApplication config saved')
  } catch (error) {
    throw new Error(`Failed while saving the application config with error: ${error.toString()}`)
  }

  console.log('\nSetup complete.  Run "npm build", then "npm start" to launch')
}
